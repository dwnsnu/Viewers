...
const style = {
  ...
  backgroundColor: 'rgba(17, 88, 176, 0.28)',
  position: 'absolute',
  // left: '0px'
  zIndex: '999'
}
...
class Histogram extends PureComponent {
  ...
  componentDidMount() {
    ...
    const element = document.querySelector('.ViewerMain');
    ...
    this.pixelData = this.props.loadImage.getPixelData();
    
    // console.log("this.props.loadImage", this.props.loadImage)
    this.updateCanvas();
  }
  ...
  getRGBPixelsImage(x, y, width, height) {
    const image = this.props.loadImage
  ...
  getPixel(x, y) {
    ...
    // const element = this.state.dicomElement;
    // const isDicom = this.state.isDicom;
    
    let sp = []
    if (this.props.isDicom) {
      if (this.props.image.color) {
        sp = getRGBPixels(this.state.dicomElement, x, y, 1, 1)
      } else {
        sp = cornerstone.getStoredPixels(this.state.dicomElement, x, y, 1, 1)
      }
    } else {
  ...
  render() {
    ...
    return (
      <div style={{ position: 'absolute', top: '0px', left: '0px', zIndex: '999' }}>
        <Draggable
          handle="strong"
          // className="histogramCanvas"
          defaultPosition={{ x: 20, y: 20 }}
          onStart={this.onStart}
          onDrag={this.onDrag}
          onStop={this.onStop}>
          <div className="box no-cursor" style={{ touchAction: 'none', transform: 'translate(50px, 50px)', backgroundColor: 'red' }}>
